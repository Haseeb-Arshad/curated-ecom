...
export async function loader({ params }: LoaderFunctionArgs) {
  ...
  try {
    ...
    if (base) {
      const res = await fetch(`${base}/api/products?category=${encodeURIComponent(slug)}&limit=60`);
      if (res.ok) {
  ...
  if (items.length === 0) {
    ...
    const proper = slug.replace(/-/g,' ');
    items = local.products.filter((p) => p.category.toLowerCase() === proper);
  }
  return { slug, items };
}
